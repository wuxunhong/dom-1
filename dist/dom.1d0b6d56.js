...
window.dom = {
  ...
  empty: function empty(node) {
    ...
    var array = []; //引用,需要创建一个空数组

    var x = node.firstChild; //找到第一个儿子

    while (x) {
      //当x存在时,我就把他移除,然后放到我的数组里面
  ...
  find: function find(selector, scope) {
    return (scope || document).querySelectorAll(selector); //意思:如果有scope,那我就在scope里面调用querySelectorAll;如果没有,那就在document里面调用querySelectorAll
  },
  ...
  siblings: function siblings(node) {
    return Array.from(node.parentNode.children) //伪数组变成数组
    .filter(function (n) {
      return n !== node;
  ...
  index: function index(node) {
    var list = dom.children(node.parentNode); //获取它爸爸的儿子,并把儿子叫list
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "3885" + '/');
